...
// --- MTypes.h Integration (Dungeon Definition) ---
struct Dungeon {
	
};
//typedef std::vector<Dungeon> Dungeon;
typedef std::vector<class Dungeon> Dungeons;
...
// --------------------------------------------------------------------------
    ...
    Dungeons loadDungeon(std::string filename) {
    Dungeons dungeons;
    // Dungeon record size is 160 bytes, inferred from MSaver.cpp
    RecordReader<20> rr(filename);
    rr.read();
	// Assuming 'version' here is actually the 'Number of Levels' as per file spec.
    uint16_t number_of_levels = rr.getWord();
    rr.read();
    rr.read();
    rr.read();
// The pointer is now exactly at the start of Level 1 data.
    // Read the first 20-byte record of the level (Level Header starts here).
    rr.read(); 

    // --- Level Header Record 1 ---
    uint16_t x_dim = rr.getWord();
    uint16_t y_dim = rr.getWord();
    uint16_t level_number = rr.getWord();
    uint16_t num_areas = rr.getWord();
    uint16_t num_chutes = rr.getWord();
    uint16_t num_teleports = rr.getWord();

    qInfo() << "x_dim:" << x_dim;	
    qInfo() << "y_dim:" << y_dim;	
    qInfo() << "level_number:" << level_number;	
    qInfo() << "num_Areas:" << num_areas;	
    qInfo() << "num chutes:" << num_chutes;	
    // Now proceed with parsing the 900 Room Data Records, 
    // calling rr.read() for every 20-byte record boundary crossed.

/*
    rr.read();
    uint16_t nDungeons = rr.getWord();
    qInfo() << "Dungeons:" << nDungeons;	
    for (size_t i = 0; i < nDungeons; i++) 
    {
      rr.read();
      qInfo() << "------------DungeonID------------:" << i;
      std::string dungeonName = rr.getString(30);
      qInfo() << "Name (30 bytes):" << QString::fromStdString(dungeonName);
...
//          qInfo() << QString("Comp %1 | Name: \"%2\", ID: %3, HP: %4/%5")
//              .arg(i).arg(QString::fromStdString(customName)).arg(monsterID).arg(currentHP_c).arg(maxHP_c);
      }
...
int main(int argc, char *argv[]) {
    ...
    QCoreApplication::setApplicationName("Dungeon Converter");
    ...
    if (inPath.isEmpty()) inPath = "MDATA11.MDR";
    ...
    // 3. Load Dungeon (Using the C++ loadDungeons function)
    Dungeons dungeons;
    try {
        dungeons = loadDungeon(inPath.toStdString()); // Call the C++ loader
    } catch (const char* msg) {
        qCritical() << "Error loading dungeons from file:" << msg;
        return 1;
    } catch (...) {
